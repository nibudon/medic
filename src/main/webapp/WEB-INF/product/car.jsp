...
<script type="text/javascript" src="${pageContext.request.contextPath}/js/libs/jquery.min.js"></script>
<%--<script type="text/javascript" src="http://api.map.baidu.com/api?v=2.0&ak=PuOGKhs0GpRer026UWZwBRHV45LnaVjP"></script>--%>
<script type="text/javascript">
$(document).ready(function(){

    $("#myAddress").change(function(){
        var addressId=$(this).children('option:selected').val();
        var data="id="+addressId;
        $.ajax({
            url : "${pageContext.request.contextPath }/address/getAddressById.html",
            type : "get",
            cache : false,
            data : data,
            dataType : "text",
            ...
            success : function(datas) {
                ...
                $("#oName").val(json.receivePerson);
                $("#oPhone").val(json.telephone);
                $("#oAddress").val(json.address);
                $("#oName").focus();
                $("#oPhone").focus();
                $("#oAddress").focus();
            },
        ...
        geolocation.getCurrentPosition(function(r){
            if(this.getStatus() == BMAP_STATUS_SUCCESS){
                ...
                geoc.getLocation(pt, function(rs){//解析格式：城市，区县，街道
                    ...
                    var location=addComp.province+addComp.city+addComp.district+addComp.street;
                    $("#oAddress").val(location);
                    //addCount(location);
			...
			<ul>
				...
				<c:if test="${sessionScope.frontuser!=null}">
					<li id="headerLogin" class="headerLogin" style="display: list-item;">
						...
						<input id="hidUid" value="${sessionScope.frontuserId}" type="hidden"/>
						<a href="javascript:void(0)">欢迎:${sessionScope.frontuser}</a>|
					</li>
				</c:if>	
				...
				<div id="carinfo" style="display: none">
					...
					<div class="span24">
						...
						收货人&nbsp;&nbsp;&nbsp;：<input id="oName" name="oName" type="text" value="" style="width:400px;height: 20px;" />
									...
									<br /> 
						联系方式：<input id="oPhone" name="oPhone" type="text" value="" style="width:400px;height: 20px;" />
												<span style="color:red" id="phonemsg"></span>
						<br />
						收货地址：<input id="oAddress" name="oAddress" type="text" value="" style="width:400px;height: 20px;" />
